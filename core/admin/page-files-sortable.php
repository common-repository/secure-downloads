...
//debuge($pro_id, $exist_product)				;
				if ( ! empty( $exist_product ) ) {
					...
					if ( isset( $_POST[ $post_key ] ) &&  isset( $_POST[ $post_key ][ $post_index ] ) ) {				
						...
						//FixIn: 1.2.3.1
						$product_arr['path'] = str_replace( array( '../', './'), '', $product_arr['path'] );    //Remove up leveling.

						// Check if path contain secret folder
						$opsd_upload = opsd_upload();
						$dir_level1  = $opsd_upload->get_protected_dir_name();
						if ( false === strpos( $product_arr['path'], $dir_level1 ) ) {
					...
					// Product  does not exist so probably  its Group section  title.
					$post_key = 'pro_title';
					if ( isset( $_POST[ $post_key ] ) &&  isset( $_POST[ $post_key ][ $post_index ] ) ) {				
						$product_arr[ 'title' ] = OPSD_Settings_API::validate_text_post_static( $post_key , $post_index );	// Validate
					}
