...
class OPSD_Page_Files extends OPSD_Page_Structure {
    ...
    public function update() {
		...
		//FixIn: 1.2.3.1
	    $validated_option = str_replace( array('../','./'), '', $validated_option ); 	//Remove up leveling.

		// Check if path contain secret folder
		$opsd_upload = opsd_upload();
		$dir_level1  = $opsd_upload->get_protected_dir_name();
			// Something wrong,  the path  does not contain secret folder

	    $products_obj = new OPSD_Products( $validated_option );
	    $products_arr = $products_obj->get_products();
	    foreach ( $products_arr as $key => $product_arr ) {
		    if ( isset( $product_arr['path'] ) ) {
			    if (
				    ( empty( $product_arr['path'] ) ) ||
				    ( false === strpos( $product_arr['path'], $dir_level1 ) )
			    ) {
				    opsd_show_message( __( 'File paths are incorrect in some products. The path must contain a secret folder: ', 'secure-downloads' ) . ' ' . $dir_level1, 5, 'error' );
				    //$products_arr[ $key ]['path'] = '';
					unset( $products_arr[ $key ] );
			    }
		    } else {
			    if ( isset( $product_arr['id'] ) ) {
				    unset( $products_arr[ $key ]['id'] );
			    }
		    }
	    }
	    $validated_option = $products_obj->get_products_for_saving( $products_arr );
