...
function opsd_bs_select( $item ) {
    ...
    ?><select  
            ...
            <?php 
				...
				$is_func_name_have_arg = strpos( $item_params['onchange'], '(' );
				if ( false !== $is_func_name_have_arg ) {
					$onchange_func_name = substr( $item_params['onchange'], 0, strpos( $item_params['onchange'], '(' ) );
				} else {
					$onchange_func_name = $item_params['onchange'];
				}
                if ( ! empty( $item_params['onchange'] ) ) {
                    ?> onchange="javascript:if ( 'function' === typeof( <?php echo $onchange_func_name; ?> )  ) { <?php
							echo $item_params['onchange'];
					?>; }" <?php
                }
            ?>
            autocomplete="off"
                                ...
                                , array(                                            
                                    ...
                                    , 'onchange' => "jQuery('.opsd_field_generator').hide();jQuery('.opsd_field_generator_' + this.options[this.selectedIndex].value ).show();"                                
                                        ...
                                        , array( 
                                            ...
                                            , 'font_icon' => 'glyphicon glyphicon-trash'
                                            , 'icon_position' => 'left'
                                            , 'action' => "jQuery('#securedownloads_filters_formID').trigger( 'submit' );"
                                        )
                    )
              );     
...
opsd_dropdown_menu( array( 
                         ...
                         , 'position' => 'right'
                         , 'items' => array( 
                                             array( 'type' => 'link', 'title' => __('About Plugin', 'secure-downloads'), 'url' => 'https://oplugins.com /' )
                                           , array( 'type' => 'divider' )
                                           ...
                                           , array( 'type' => 'link', 'title' => __('Technical Support', 'secure-downloads'), 'url' => 'https://oplugins.com/plugins/secure-downloads/support/' )
                                           , array( 'type' => 'divider' )
                                           , array( 'type' => 'link', 'title' => __('Upgrade Now', 'secure-downloads'), 'url' => 'https://oplugins.com/', 'style' => 'font-weight: 600;' , 'attr' => array( 'target' => '_blank' ) )
                                      )
                  ) ); 
 * 
 * @param array $args
