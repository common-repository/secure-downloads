...
//debuge($product , $opt)		;die;
		...
		if ( isset( $opt[ 'expire' ] ) )	{			                                                                //FixIn: 1.2.2.2
...
//die;
			...
			// Check IP /////////////////////////////////////////////////////////
			$download[ 'ip' ] = apply_filters( 'opsd_define_ip_mask' , $download[ 'ip' ] );                             //FixIn: 1.1.4.2
			if ( ! is_array( $download['ip'] ) ) {
				...
				$is_ip_valid = false;
				$ip_real     = opsd_get_user_ip();
				foreach ( $download['ip'] as $ip_mask_single ) {
					if ( $ip_mask_single == $ip_real ) {
						$is_ip_valid = true;
					}
				}
				if ( ! $is_ip_valid ) {
